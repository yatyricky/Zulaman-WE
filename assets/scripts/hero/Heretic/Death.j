...
constant string  ART_TARGET  = "Abilities\\Spells\\Undead\\AnimateDead\\AnimateDeadTarget.mdl";
    ...
    function onCast() {
        ...
        DamageTarget(SpellEvent.TargetUnit, SpellEvent.CastingUnit, dmg, SpellData.inst(SID_DEATH, SCOPE_PREFIX).name, false, false, false, WEAPON_TYPE_WHOKNOWS);
        dmg = returnDamage(lvl, dmg, UnitProp.inst(SpellEvent.CastingUnit, SCOPE_PREFIX).SpellPower());
        DamageTarget(SpellEvent.CastingUnit, SpellEvent.TargetUnit, dmg, SpellData.inst(SID_DEATH, SCOPE_PREFIX).name, false, true, false, WEAPON_TYPE_WHOKNOWS);
