...
constant integer BUFF_ID = 'A08D';
    ...
    function onEffect(Buff buf) {
        UnitProp.inst(buf.bd.target, SCOPE_PREFIX).ModAgi(buf.bd.i0);
    }
    
    function onRemove(Buff buf) {
        UnitProp.inst(buf.bd.target, SCOPE_PREFIX).ModAgi(0 - buf.bd.i0);
    }
    ...
    function onInit() {
        RegisterSpellEffectResponse(SID_AGILITY_POTION, onCast);
