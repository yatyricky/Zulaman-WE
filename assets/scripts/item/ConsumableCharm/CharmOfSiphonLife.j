...
constant string  ART_CASTER  = "Abilities\\Spells\\Other\\Drain\\DrainCaster.mdl";
    ...
    function onEffect(Buff buf) {
        DamageTarget(buf.bd.caster, buf.bd.target, buf.bd.r0, SpellData.inst(SID_CHARM_OF_SIPHON_LIFE, SCOPE_PREFIX).name, false, true, false, WEAPON_TYPE_WHOKNOWS);
        HealTarget(buf.bd.caster, buf.bd.caster, DamageResult.amount, SpellData.inst(SID_CHARM_OF_SIPHON_LIFE, SCOPE_PREFIX).name, -3.0);
