            ...
            if (ModuloInteger(cd.nodes, 5) == 0) {
                for (0 <= i < PlayerUnits.n) {
                    ...
                    if (DistancePointAndLineSegment(data.origin, data.v1, data.v2) < 100.0) {
                        DamageTarget(cd.caster, PlayerUnits.units[i], GetWidgetLife(PlayerUnits.units[i]) * 1.3, SpellData.inst(SID, SCOPE_PREFIX).name, false, true, false, WEAPON_TYPE_WHOKNOWS, false);
                    }
                }
            }
