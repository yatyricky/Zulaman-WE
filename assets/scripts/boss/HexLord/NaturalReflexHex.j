...
// constant string  ART  = "Abilities\\Spells\\Human\\Invisibility\\InvisibilityTarget.mdl";
    ...
    function hexLordHitted() {
        ...
        if (DamageResult.isDodged) {
            ...
            if (ilvl > 0) {
                HealTarget(DamageResult.target, DamageResult.target, GetUnitState(DamageResult.target, UNIT_STATE_MAX_LIFE) * 0.1, SpellData.inst(SID_NATURAL_REFLEX_HEX, SCOPE_PREFIX).name, -3.0, false);
