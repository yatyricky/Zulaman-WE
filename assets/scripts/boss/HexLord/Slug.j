...
library Slug requires DamageSystem {
    ...
    function onEffect(Buff buf) {
        ...
        // max limit
        if (list.count() > 13) {
            DestroyEffect(Int2Eff(list.removeNode(list.head)));
        }
        // do damage
        iter = list.head;
        while (iter != 0) {
            ...
            while (i < PlayerUnits.n) {
                if (GetDistance.unitCoord2d(PlayerUnits.units[i], BlzGetLocalSpecialEffectX(eff), BlzGetLocalSpecialEffectY(eff)) < 150.0 && damaged.exists(PlayerUnits.units[i]) == false) {
