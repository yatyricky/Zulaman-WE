...
constant integer BUFF_ID = 'A011';
    ...
    function onEffect(Buff buf) {
        if (buf.bd.tick != 1) {
            HealTarget(buf.bd.caster, buf.bd.target, 1000.0, SpellData.inst(SID_LIFE_BLOOMHEX, SCOPE_PREFIX).name, 0.0, false);
            AddTimedEffect.atUnit(ART_HEAL, buf.bd.target, "origin", 0.2);
        }
    }

    function onRemove(Buff buf) {
        HealTarget(buf.bd.caster, buf.bd.target, 2000.0, SpellData.inst(SID_LIFE_BLOOMHEX, SCOPE_PREFIX).name, 0.0, false);
        AddTimedEffect.atUnit(ART_HEAL, buf.bd.target, "origin", 0.2);
        //BJDebugMsg("����");
    }
