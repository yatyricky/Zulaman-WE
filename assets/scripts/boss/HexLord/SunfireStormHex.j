            ...
            if (IsInCombat()) {
                while (i < PlayerUnits.n) {
                    if (GetDistance.unitCoord(PlayerUnits.units[i], this.p.x, this.p.y) < HexLordGlobalConst.sunFireAOE && !IsUnitDead(PlayerUnits.units[i])) {
                        DamageTarget(this.caster, PlayerUnits.units[i], this.dmg, SpellData.inst(SID_SUN_FIRE_STORMHEX, SCOPE_PREFIX).name, false, false, false, WEAPON_TYPE_WHOKNOWS, false);
                    }
                    i += 1;
                }
            }
