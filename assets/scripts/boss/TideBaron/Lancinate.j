...
constant integer BUFF_ID = 'A050';
    function onEffect(Buff buf) {
        ...
        DamageTarget(buf.bd.caster, buf.bd.target, amt, SpellData.inst(SID_LANCINATE, SCOPE_PREFIX).name, true, false, false, WEAPON_TYPE_WHOKNOWS);
    ...
    function onCast() {
        Buff buf;
        DamageTarget(SpellEvent.CastingUnit, SpellEvent.TargetUnit, 450.0 + GetRandomReal(0.0, 100.0), SpellData.inst(SID_LANCINATE, SCOPE_PREFIX).name, true, false, true, WEAPON_TYPE_METAL_LIGHT_SLICE);
