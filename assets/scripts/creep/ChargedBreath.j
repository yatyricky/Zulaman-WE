    ...
    function onCast() {
        ...
        while (i < PlayerUnits.n) {
            if (GetDistance.units2d(PlayerUnits.units[i], SpellEvent.CastingUnit) <= returnRange() && !IsUnitDead(PlayerUnits.units[i])) {                
                DamageTarget(SpellEvent.CastingUnit, PlayerUnits.units[i], GetUnitMana(SpellEvent.CastingUnit), SpellData.inst(SID_CHARGED_BREATH, SCOPE_PREFIX).name, false, false, false, WEAPON_TYPE_WHOKNOWS);
