    ...
    function maidOfAgonyDamaged() {
        ...
        if (GetUnitTypeId(DamageResult.target) == UTID_MAID_OF_AGONY) {
            if (MarkOfAgony.ht.exists(DamageResult.target)) {
                ...
                for (0 <= i < data.n) {
                    DamageTarget(DamageResult.target, data.targets[i], DamageResult.amount * 0.25, SpellData.inst(SID_MARK_OF_AGONY, SCOPE_PREFIX).name, false, false, false, WEAPON_TYPE_WHOKNOWS, false);
                }
            }
        }
    }
