...
library ShadowSpike requires DamageSystem {
    ...
    function onEffect(Buff buf) {
        ...
        DamageTarget(buf.bd.caster, buf.bd.target, amt, SpellData.inst(SID_SHADOW_SPIKE, SCOPE_PREFIX).name, false, false, false, WEAPON_TYPE_WHOKNOWS, false);
    ...
    function onhit(Projectile p) -> boolean {
        ...
        if (TryReflect(p.target)) {
            ...
            DamageTarget(p.caster, p.target, 500.0, SpellData.inst(SID_SHADOW_SPIKE, SCOPE_PREFIX).name, false, true, false, WEAPON_TYPE_WHOKNOWS, false);
