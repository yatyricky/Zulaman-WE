    ...
    function onCast() {
        ...
        if (!IsUnitEnemy(SpellEvent.TargetUnit, GetOwningPlayer(SpellEvent.CastingUnit))) {
            ...
            while (i < num) {
                ...
                if (buf != 0) {
                    if (!IsUnitDead(buf.bd.caster)) {     
                        ...
                        DamageTarget(SpellEvent.CastingUnit, buf.bd.target, 100 + UnitProp.inst(SpellEvent.CastingUnit, SCOPE_PREFIX).SpellPower(), SpellData[SID_SPELL_TRANSFER].name, false, true, false, WEAPON_TYPE_WHOKNOWS);  
                    }
                } else {
                    i += 999;
                }
            ...
            while (i < num) {
                ...
                if (buf != 0) {                        
                    DamageTarget(SpellEvent.CastingUnit, SpellEvent.TargetUnit, 100 + UnitProp.inst(SpellEvent.CastingUnit, SCOPE_PREFIX).SpellPower(), SpellData[SID_SPELL_TRANSFER].name, false, true, false, WEAPON_TYPE_WHOKNOWS);  
